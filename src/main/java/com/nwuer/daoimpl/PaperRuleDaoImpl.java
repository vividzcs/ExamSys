...
public class PaperRuleDaoImpl extends BaseDaoImpl<PaperRule> {
	...
	public int getIdByMajorAndSubject(int m_id,int sub_id) {
		List<PaperRule> list = (List<PaperRule>) this.getHibernateTemplate().find("select p_id from PaperRule where major.m_id=? and subject.sub_id=?", m_id,sub_id);
		if(list != null && list.size()>0) {
			return list.get(0).getP_id();
		}else
			return 0;
	}
}
